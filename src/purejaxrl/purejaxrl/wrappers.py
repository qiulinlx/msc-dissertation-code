...
class UpscaleMinAtarObservationWrapper(GymnaxWrapper):
    """Flatten the observations of the environment."""

    def __init__(self, env: environment.Environment, resize_method: str = "nearest"):
        super().__init__(env)
        self.new_obs_shape = (30, 30, 4)
    ...
    def reset(
        ...
        obs, state = self._env.reset(key, params)
        obs = jax.image.resize(obs, shape=self.new_obs_shape, method=self.resize_method)
        return obs, state
