...
def make_ppo_train(config):
    ...
    def train(rng):
        ...
        init_x = (
            jnp.zeros(
                (1, config["NUM_ENVS"], *envs.single_observation_space.shape)
            ),
        ...
        def _update_step(runner_state, unused):
            ...
            # CALCULATE ADVANTAGE
            (
                train_state, env_state, episode_stats, last_obs, last_mask, last_done, hstate, rng
            ) = runner_state
            ...
            runner_state = (train_state, env_state, episode_stats, last_obs, last_mask, last_done, hstate, rng)
...
def make_agent_evaluation(config):
    ...
    def agent_eval(params):
        def _env_step(runner_state, unused):
            ...
            # FORWARD PASS
            ac_in = (last_obs[np.newaxis, :], last_done[np.newaxis, :], last_mask[np.newaxis, :])
            hstate, pi, mu, value = network.apply(params, hstate, ac_in)  # mu is mask distribution
            ...
            transition = Transition(
                done, action, value, reward, log_prob, last_obs, last_mask, maskv, info, mu.probs
                # last_mask -> mask_in, maskv -> mask_out
            )
            runner_state = (params, env_state, episode_stats, obsv, maskv, done, hstate, rng)
            return runner_state, transition
