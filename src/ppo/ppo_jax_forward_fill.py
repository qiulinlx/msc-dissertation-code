...
from src.agents.jax_agents_ff import (
    RegularAgentDense,
    RegularAgentCNN,
)
...
def make_ppo_train(config):
    ...
    if config['AGENT'] == "RegularAgentDense":
    ...
    def train(rng):
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                train_state, env_state, last_obsff, last_done, hstate, rng = runner_state

                # SELECT ACTION (NB: agent gets last_obsff, not last_obs)
                rng, _rng = jax.random.split(rng)
                ac_in = (last_obsff[np.newaxis, :], last_done[np.newaxis, :])
                ...
                # UPDATE FORWARD FILL
                rng, _rng = jax.random.split(rng)
                mask = network.generate_mask(
                    _rng,
                    obsv.shape,
                    config["AGENT_CONFIG"].get("mask_ratio"),
                    config["AGENT_CONFIG"].get("patch_size", None),  # only used by AttentionAgentCNN
                    False  # noise masking - only used by RegularAgent*
                )
                obsffv = obsv * mask + last_obsff * (1 - mask)
                obsffv = jnp.where(done[..., None], obsv, obsffv)  # todo: make this work for vision envs too

                transition = Transition(
                    done, action, value, reward, log_prob, last_obsff, info
                )
                runner_state = (train_state, env_state, obsffv, done, hstate, rng)
...
def make_agent_evaluation(config):
    ...
    def agent_eval(params):
        def _env_step(runner_state, unused):
            params, env_state, last_obsff, last_done, hstate, rng = runner_state
            ...
            obsffv = obsv * mask + last_obsff * (1 - mask)
            obsffv = jnp.where(done[..., None], obsv, obsffv)

            transition = Transition(
                done, action, value, reward, log_prob, last_obsff, info
            )
            runner_state = (params, env_state, obsffv, done, hstate, rng)
            return runner_state, transition
